...
const { applyDecimals, web3 } = require('../../Containers/Metamask/utils/ethereumAPI');
...
const ERC20Create = ({ importToken }) => {
    ...
    const onClickAction = async () => {
        ...
        try {
            ...
            axios.post('http://localhost:7000/api/postAddress', { contractHashAddress: result._address });
        } catch (error) {
    ...
    disabled={loading}
    component={Link}
    to={`/AddCar?tokenAddress=${web3Token.options.address}`}
>
    {successMessage ? "Token info" : (loading ? <CircularProgress size={25} /> : "send")}
</Button>
